var PokedexPokemonPanel = PokedexResultPanel.extend({
	initialize: function(id) {
		...
		buf += '<h1>';
		if (pokemon.forme && (pokemon.isNonstandard !== 'Fundex' || pokemon.name.includes('-'))) {
		...
		if (pokemon.isNonstandard) {
			...
			} else if (pokemon.num > 0 && pokemon.isNonstandard !== 'Fundex') {
	...
	getEvoMethod: function(evo) {
		...
		case 'trade':
			if (evo.evoItem) {
				return 'trade holding ' + evo.evoItem + condition;
			}
			return 'trade';
		case 'other':
	...
	renderDetails: function() {
		...
		var digitalFaintCry = ['Blue Screen', 'Internet Explorer', 'Inori Aizawa', 'Microsoft Edge', 'Mozilla Firefox', 'Google Chrome', 'Recycle Bin'];
