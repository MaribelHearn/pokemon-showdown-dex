var PokedexSearchPanel = Panels.Panel.extend({
	...
	initialize: function () {
		...
		buf += '<h1><a href="/" data-target="replace">Fundex</a></h1>';
		...
		if (fragment === '') {
			buf += '<p class="buttonbar"><button class="button"><strong>Fundex Search</strong></button> <button name="lucky" class="button">I\'m Feeling Lucky</button></p>';
		}
		buf += '</form>';
	...
	click: function(e) {
		...
		if (e.target.tagName === 'BUTTON' && e.target.name === 'lucky') {
			e.preventDefault();
			e.stopPropagation();
			alert(['Who do you think you are?','Chuck Norris is feeling lucky.','Zeeky boogy doog. *explodes*','The Game','Congratulations! You are the 100 millionth clicker and you just won a free iPod!'][Math.floor(Math.random()*4)]);
			return;
		}
